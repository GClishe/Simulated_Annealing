...
#Tunable parameters
dataName = 'Place_20000'         #Name of netlist file. Need to add .py to the end of provided files. Make sure original folder names are used and that result folders exist
...
tempCount = 0                   #Number of temperature steps to cycle through between initial and final temp during the geometric cooling cycle. Leave at 0 for ~1 hour run on lab machines
...
if tempCount == 0:                              #If tempCount set to 0 (auto)
    if folderName == 'Ptest_Tests':             #Use adaptive tempCount for Ptest and Place vs grid size to get arround 1 hour of execution time
        tempCount = 0.95 * 4153733.94378806*math.e**(-0.0146904901*data['grid_size'])
    else:
        tempCount = 0.95 * 3693057.28666504*math.e**(-0.0049358788*data['grid_size'])
...
print(f'\nT0: {T:.1f}\nTf: {T_min:.3f}\nBoltzman Constant (K): {K_BOLTZ:.3f}\nCooling Rate: {coolRate:.8f}\nIterations Per Temp Step: {MOVES_PER_T_STEP:.0f}\nTemperature Step Count: {tempCount:.0f}\nRandom Move Chance: {curr_random_move_chance:.2f}')
